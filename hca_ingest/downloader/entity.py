...
@dataclass
class Entity:
    entity_json: dict = field(init=True, default_factory=dict)
    content: dict = field(init=False, default_factory=dict)
    links: dict = field(init=False, default_factory=dict)
    ...
    def __init__(self, entity_json: dict = None):
        self.entity_json = entity_json if entity_json else {}
        self.content = self.__get_item(entity_json, 'content', {})
        self.links = self.__get_item(entity_json, '_links', {})
    ...
    def set_input(self, input_biomaterials=None, input_files=None, process: Entity = None,
                  protocols: list[Entity] = None):
